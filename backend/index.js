...
mongoose.connect(process.env.MONGODB_URI || process.env.MONGODB_URL || "mongodb+srv://kushvaha5678_db_user:Shivam123@cluster0.9gj2exy.mongodb.net/")
...
// MiddleWare to fetch user from token (returns early on failure)
...
const ensureDbConnected = (req, res, next) => {
  if (mongoose.connection.readyState !== 1) {
    return res.status(503).json({ errors: "Database is not connected - try again later" });
  }
  next();
};
...
app.post('/signup', ensureDbConnected, async (req, res) => {
  ...
  try {
    ...
    if (!email || !password || !username) {
      return res.status(400).json({ success, errors: 'Please provide username, email and password' });
    }
